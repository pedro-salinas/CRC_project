...
export function ClientsModalInsert({
    ...
    // Handler para ingresar cliente
    const onSubmit = handleSubmit(async (values) => {
        setLoading(true);
        const backendValidation = async () => {
            try {
                const res = await insertClientRequest(values);

                setLoading(false);

                setAlertType("success");
                setAlertText("Paciente ingresado correctamente");
                setShowAlert(true);
        ...
        <Modal show={show} onHide={handleClose}>
            <Form className="p-2" onSubmit={onSubmit}>
                <Modal.Header closeButton disabled={loading}>
                    <Modal.Title>Ingresar paciente</Modal.Title>
                </Modal.Header>
                <Modal.Body>
