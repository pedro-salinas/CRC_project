...
export function AttentionsCalendarModalInsertMultipleDifferentDate({
    ...
    const onSubmit = handleSubmit(async (values) => {
        ...
        const backendValidation = async () => {
            try {
                ...
                allDates.push([day, month, year, hour]);

                const sortByNumberAfterDash = (a, b) => {
                ...
                for (let index = 0; index < dates.length; index++) {
                    const valueHour = values[hours[index]];
                    const valueDate = values[dates[index]];

                    const hour = Number(valueHour);
                    const day = Number(valueDate.split("-")[2]);
                    const month = Number(valueDate.split("-")[1]);
                    const year = Number(valueDate.split("-")[0]);
    ...
    const removeAdditionalContent = () => {
        if (numAdditionalContent != 0) {
            unregister(`date-${numAdditionalContent - 1}`);
            unregister(`hour-${numAdditionalContent - 1}`);
            setNumAdditionalContent(numAdditionalContent - 1);
        }
    };
                    ...
                    <Row>
                        <Col>
                            <Form.Group className="mb-3" controlId="date">
                                <Form.Label>Fecha (Día 1)</Form.Label>
                                <input
                                    type="date"
                                    className={
                                        errors.date
                        ...
                        <Col>
                            <Form.Group className="mb-3" controlId="hour">
                                <Form.Label>Hora (Día 1)</Form.Label>
                                <select
                                    className={
                                        errors.hour
                                            ? "form-control is-invalid"
